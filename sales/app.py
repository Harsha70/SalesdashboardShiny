...
with ui.card():
    ui.card_header("Aggregate sales over time")
    
    @render_plotly
    def sales_over_time_plot():
        df = dat()
        sales_over_time = df.groupby(df['order_date'].dt.to_period('M')).agg({'price_each': 'sum'}).reset_index()
        sales_over_time['order_date'] = sales_over_time['order_date'].dt.to_timestamp()
        fig = px.line(sales_over_time, x='order_date', y='price_each', title='Sales Over Time')
        return fig

with ui.card():
    # Aggregate quantity ordered per product
    ui.card_header("product_performance_plot")
    @render_plotly
    def product_performance_plot():
        product_performance = dat().groupby('product').agg({'quantity_ordered': 'sum'}).reset_index()
        fig = px.bar(product_performance, x='product', y='quantity_ordered', title='Product Performance')
        return fig
...
with ui.card():
    ui.card_header("Sales by Product and City")
    @render_plotly
    def sales_product_city_plot():
        sales_product_city = dat().groupby(['product', 'city']).agg({'price_each': 'sum'}).reset_index()
        fig = px.bar(sales_product_city, x='product', y='price_each', color='city', title='Sales by Product and City')
        return fig
